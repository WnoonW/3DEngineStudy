...
bool App::Initialize()
{
    ...
    CreateUIObject(XMFLOAT3(0,0,0), XMFLOAT3(1, 1, 1));
...
void App::Update(const GameTimer& gt)
{
    ...
    for (auto& obj : mObjects) {
        if (!obj->isUI) {
            auto& transform = mRegistry.GetComponent<TransformComponent>(obj->mEntityID);
            obj->SetPosition(transform);
        }
    }
    ...
    {
        if (obj->isUI)
            obj->Update(gt, XMMatrixIdentity(), XMMatrixOrthographicLH(2.0f, 2.0f, 0.0f, 1.0f));
        else
            obj->Update(gt, view, proj);
...
void App::CreateObject(XMFLOAT3 pos, XMFLOAT3 scale)
{
	...
	mRegistry.AddComponent(entity, TransformComponent{ pos, scale , { 0.0f, 0.0f, 0.0f } });
    mRegistry.AddComponent(entity, GravityComponent{-9.8f, true});
    mObjects.emplace_back(std::make_unique<Object>(mResourceManager.get(), pos, scale, false, entity));
}
...
void App::CreateUIObject(XMFLOAT3 pos, XMFLOAT3 scale)
{
    Entity entity = mRegistry.CreateEntity();
    mRegistry.AddComponent(entity, TransformComponent{ pos, scale , { 0.0f, 0.0f, 0.0f } });
    mObjects.emplace_back(std::make_unique<Object>(mResourceManager.get(), pos, scale, true, entity));
}

void App::DestroyObject(Object* object)
{
    if (!object) return;  // nullptr 방지

    // vector에서 해당 오브젝트 제거 (unique_ptr이므로 자동 해제)
    auto it = std::find_if(mObjects.begin(), mObjects.end(),
        [object](const std::unique_ptr<Object>& ptr) { return ptr.get() == object; });

    if (it != mObjects.end())
    {
        mObjects.erase(it);  // erase로 제거 (unique_ptr 소멸자 호출)
        OutputDebugStringA("Object destroyed!\n");  // 디버그 로그 (선택)
    }

    mSelectedObject = nullptr;  // 선택 해제
}
